...
= Patch386
=
= Stub - ARM64 always has 32-bit (and 64-bit) instructions
=
========================
*/
...
boolean SaveTheGame(int file,int x,int y)
{
	...
	// Skip disk space check - not applicable on modern systems
...
void InitGame (void)
{
	...
	{
		ShutdownId();
		fprintf(stderr, "Insufficient memory to run Wolfenstein 3D.\n");
		exit(1);
	}
...
//
	...
	// Single framebuffer - no page flipping needed
	displayofs = 0;
	bufferofs = 0;
}
