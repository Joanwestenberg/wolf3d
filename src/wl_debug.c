...
#include "wl_def.h"
#pragma hdrstop
// #include <bios.h> -- removed for macOS port
...
//
	...
	// In SDL2 port, just wait for a key and quit
	IN_Ack();
	Quit (NULL);
}
...
void ShapeTest (void)
{
#if 0 // Disabled in SDL2 port - uses old page manager internals
static	char	buf[10];
...
#endif
			case actoratview:
				tile = ACTORAT_TO_INT(actorat[x][y]);
				break;
			}
