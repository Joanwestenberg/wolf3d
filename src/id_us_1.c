...
//
// backcolor global - used by text printing routines
//
byte backcolor;

//
// Table-driven pseudo-random number generator (from ID_US_A.ASM)
//
static int rndindex;
...
static	WindowRec	wr;
	...
	di = 0;  // _DI register not available on modern systems
	...
	c = 3;	// Assume text mode (macOS port)
	if ((c < 4) || (c == 7))
		goto oh_kill_me;
	...
	// sti removed - not needed on modern systems
...
US_Startup(void)
{
	...
	// harderr not available on modern systems (DOS error handler)
